...
import { TimebarService } from "../../../visuall/timebar.service";
import {
  ClassOption,
  Rule,
  RuleSync,
...
import { GroupTabComponent } from "../../../visuall/operation-tabs/map-tab/group-tab/group-tab.component";
import { MergedElemIndicatorTypes } from "../../../visuall/user-preference";
...
export class ClusterNodeComponent implements OnInit {
  ...
  changeSelectedClass() {
    ...
    if (isNodeClassSelected) {
      ...
      if (txt == "Commit") {
        this.selectedClassProps.push(...["Author", "PullRequest"]);
      } else if (txt == "PullRequest") {
        this.selectedClassProps.push(...["headBranch",]);
      } else if (txt == "Issue") {
        this.selectedClassProps.push(...["issueType", "priority", "reporter", "assigner", "assignee", "resolver", "closer"]);
      } else if (txt == "Developer") {
        //this.selectedClassProps.push(...["Author", "PullRequest"]);       
      } else if (txt == "File") {
        this.selectedClassProps.push(...["Folder"]);
      }
      isGeneric = false;
    }
  ...
  handleRuleReady(rule: Rule) {
    // Handle the rule emitted from the child component
  }
  ...
  runQuery() {
    ...
    this._g.cy.nodes().filter((node) => {
      if (node.classes().includes(this.selectedClass)) {
        if (this.selectedProperty in node._private.data) {
          let groupName =node._private.data[this.selectedProperty];
          if (!folderNames.includes(groupName)) {
            folderNames.push(groupName)
            clusters[groupName] = [];
            this._cyService.addParentNode(groupName);
          }  
          this._g.cy.elements(`[id = "${node.id()}"]`).move({ parent: 'c' + groupName });

          clusters[groupName].push(...this._g.cy.elements(`[id = "${node.id()}"]`)[0]._private.edges);
        } else {
          if (this.selectedProperty == "Author") {
            let groupNames = node._private.edges
              .filter((edge) => {
                // Assuming the edge has a target node and a label
                console.log(Array.from(edge._private.classes) )
                return Array.from(edge._private.classes)[0] == "COMMITTED";
              })
              .map((edge) => {
                // Assuming the edge label is stored in a property called 'label'
                return edge._private.source._private.data.name;
              })
              let groupName = groupNames[0];
  ...
  filterElesByClass(e: { className: string; willBeShowed: boolean }) {
    if (e.willBeShowed) {
      ...
      if (this._g.cy.$("." + e.className).length > 0) {
  ...
  private filter4CompoundNode(node, classCSS: string, isShow: boolean) {
    ...
    let clusterSelector = '';
    const compoundNodes = this._g.cy.nodes('.' + CLUSTER_CLASS);
