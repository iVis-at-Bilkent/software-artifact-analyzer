...
export class MissingPriorityComponent implements OnInit, QueryComponent<Anomaly> {
  ...
  loadGraph(skip: number, filter?: TableFiltering) {
    ...
    const isClientSidePagination = this._g.userPrefs.queryResultPagination.getValue() == 'Client';   
    let fn = (x) => { cb(x); this._g.add2GraphHistory(`Get Anomalies: Missing priority bugs`); };
    const cb = (x) => {
  ...
  getDataForQueryResult(e: TableRowMeta) {
    const cb = (x) => {
      ...
      this._g.add2GraphHistory(`Get Anomalies: Missing priority bugs (${names.join(", ")})`);
    }
