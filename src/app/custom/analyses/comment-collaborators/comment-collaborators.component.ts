...
import { LONG_MAX, LONG_MIN } from 'src/app/visuall/constants';
...
@Component({
  selector: 'app-comment-collaborators',
  templateUrl: './comment-collaborators.component.html',
  styleUrls: ['./comment-collaborators.component.css']
})
export class CommentCollaboratorsComponent implements OnInit, QueryComponent<Collaborator>  {
  ...
  loadGraph(skip: number, filter?: TableFiltering) {
    ...
    const txtCondition =this._h.getQueryCondition4TxtFilter(filter, ['Developer'], isIgnoreCase);
    const ui2Db = {'name': 'n.name' };
    const orderExpr =this._h. getOrderByExpression4Query(filter, 'collaborationScore', 'desc', ui2Db);
    const f1 = this.dateFilterFromUserPref('n', true);
    const f2 = this.dateFilterFromUserPref('issue', true);
    const f3 = this.dateFilterFromUserPref('d1', true);
    let f = '';
