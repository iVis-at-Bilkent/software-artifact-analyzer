...
export class CollaboratorsComponent implements OnInit {
  ...
  constructor(private _dbService: Neo4jDb, private _cyService: CytoscapeService, private _g: GlobalVariableService, 
    private _group: GroupCustomizationService, private _h: QueryHelperService) {
  }
  ...
  loadTable(skip: number, filter?: TableFiltering) {
    ...
    const orderExpr =this._h. getOrderByExpression4Query(filter, 'collaboration', 'desc', ui2Db);
  ...
  loadGraph(skip: number, filter?: TableFiltering) {
   ...
   WHERE d1 <> n  
   MATCH p = (d1)-[]->(issue:Issue)<-[]-(n) ${f} 
   WITH n, COUNT(DISTINCT issue) AS score, collect(p) AS paths, d1
   ORDER BY ${orderExpr}
   LIMIT ${this.number} 
   RETURN d1, paths;
  `
